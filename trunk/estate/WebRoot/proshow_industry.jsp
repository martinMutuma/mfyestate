...
<script type="text/javascript" src="<%=basePath %>proShow.js"></script>
  ...
  <body  onload="load()" onunload="GUnload()">
  	...
  	<div style="width: 100%;" align="center">
  	    <div class="l_loca">
    	<span class="left lo_le">您当前的位置：<a href="home.shtml">梦房园</a>&nbsp;&gt;&gt;&nbsp;<a href="search.shtml?searchPro=3"><estateTag:convertCode codeType="导航" code="${menuIndex}"></estateTag:convertCode></a>&nbsp;&gt;&gt;&nbsp;<s:property value="dataMap.title"/></span>
       </div>
	   				...
	   				<div id="baseinfo">
	   					...
	   					<div id="biRight">
	   						<table width="100%" cellspacing="0" border="0" cellpadding="0">
	   							...
	   							<tr>
	   								<td valign="bottom" ><s:if test='%{dataMap.flag == "1"}'>年租金：<estateTag:format value="${dataMap.bursary}"></estateTag:format> 万元/年</s:if><s:else>出售总价：<estateTag:format value="${dataMap.bursary}"></estateTag:format> 万元</s:else></td>
	   								<td valign="bottom">建筑面积：<estateTag:format value="${dataMap.constructionArea}"/>㎡</td>
	   							</tr>
	   							<tr>
	   								<td valign="bottom" width="50%">占地面积：<estateTag:format value="${dataMap.area}"/>㎡</td>
	   								<td valign="bottom">空地面积：
	   									<estateTag:format value="${dataMap.spaceArea}"></estateTag:format>㎡
	   								</td>
	   							</tr>
	   							<tr>
	   								<td valign="bottom">容积率：<estateTag:format value="${dataMap.far}"></estateTag:format>%</td>
	   								<td valign="bottom">绿化率：<estateTag:format value="${dataMap.greenRate}"></estateTag:format>%</td>
	   							</tr>
	   							<tr>
	   								<td valign="bottom">行业：<estateTag:convertCode codeType="工业行业" code="${dataMap.industry}"></estateTag:convertCode></td>
	   								<td valign="bottom">种类：<estateTag:convertCode codeType="工业种类" code="${dataMap.category}"></estateTag:convertCode></td>
	   							</tr>
	   							<tr>
	   								<td valign="bottom">类型：<estateTag:convertCode codeType="工业类型" code="${dataMap.type}"></estateTag:convertCode></td>
	   								<td valign="bottom">层高：${dataMap.floorHeight}</td>
	   							</tr>
	   							<tr>
	   								<td valign="bottom">行车：${dataMap.car}</td>
	   								<td valign="bottom">结构：${dataMap.structure}</td>
	   							</tr>
	   							<tr>
	   								<td valign="bottom">承重：${dataMap.bear}</td>
	   								<td valign="bottom">配电功率：<estateTag:format value="${dataMap.power}"></estateTag:format>千瓦</td>
	   							</tr>
	   							<tr>
	   								<td valign="bottom">水：<estateTag:format value="${dataMap.water}"></estateTag:format>吨</td>
	   								<td valign="bottom">通讯情况：${dataMap.communicate}</td>
	   							</tr>
	   						</table>
	   					</div>
	   				</div>
