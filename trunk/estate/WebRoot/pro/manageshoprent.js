...
$(function() {
	...
	$('#shoprentgrid').datagrid({
		...
		toolbar : [{
					...
					iconCls : 'icon-add1',
					handler : function() {
						$(this).attr("href", 'shoprent.jsp?menuCls=14');
...
function onSearch() {
      ...
      if(arear1==-1 && arear2!=-1){
       ...
       if(parseInt(arear1)>parseInt(arear2)){
         alert('选择的面积不符合要求，面积最小的值应小于面积最大的值');
         return;
        }
       if(price1==-1 && price2!=-1){
         alert('选择的租金不符合要求，请选择租金最小的值');
         return;
      }
      
       if(parseInt(price1)>parseInt(price2)){
         alert('选择的租金不符合要求，租金最小的值应小于租金最大的值');
         return;
        }
	var queryParams = $('#shoprentgrid').datagrid("options").queryParams;
...
function Onmodify(id) {
	...
	window.location.href = "../shop/searchById.shtml?id=" + id + "&type=1menuCls=14";
}
