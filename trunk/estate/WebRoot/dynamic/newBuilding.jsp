...
<html>
  ...
  <!-- 3.添加class="layoutTab"  -->
   ...
   <table width="100%" border="0" cellpadding="0" cellspacing="1" bgcolor="#CCCCCC">
      <tr>
        <td height="30" align="left" bgcolor="#9EC4ED" ><STRONG><font size="3px">&nbsp;最新开盘</font></STRONG>  </td>
      </tr>
      <tr>
...
<div class="manu" style="margin-top: 10px;text-align: right;"><span class="eg"><input name="对比选中" type="button" value="对比选中" class="contrast" onClick="compareBuildings();"/></span>
第${currentPage}页 共${rowCount}页
<s:if test="%{currentPage!=1}">
	<a href="<%=basePath%>proInfo/proInfo_showNewBuilding.shtml?page=1">首页</a>
</s:if>
<s:else>
	<font disabled="true">首页</font>
</s:else>
<s:if test="%{currentPage<rowCount}">
	<a href="<%=basePath%>proInfo/proInfo_showNewBuilding.shtml?page=${currentPage+1}">下一页</a>
</s:if>
<s:else>
	<font disabled="true">下一页</font>
</s:else>
<s:if test="%{currentPage!=1}">
	<a href="<%=basePath%>proInfo/proInfo_showNewBuilding.shtml?page=${currentPage-1}">上一页</a>
</s:if>
<s:else>
	<font disabled="true">上一页</font>
</s:else>
<s:if test="%{currentPage<rowCount}">
<a href="<%=basePath%>proInfo/proInfo_showNewBuilding.shtml?page=${rowCount}">尾页</a>
</s:if>
<s:else>
	<font disabled="true">尾页</font>
</s:else>
转到第:<input type="text" id="gotoPage" name="page" size="8"/> 页 <input id="compareButton" type="button" onclick="gotoPage();" value="GO" name="cndok" <s:if test="%{rowCount==1}"> disabled="true" </s:if>/>
</div>
</td></tr></table>
</div>
  ...
  var basePath = '<%=basePath%>';
   function gotoPage(){
	  	...
	  	window.location.href="<%=basePath%>proInfo/proInfo_showNewBuilding.shtml?page="+$("#gotoPage").val();
	  }
