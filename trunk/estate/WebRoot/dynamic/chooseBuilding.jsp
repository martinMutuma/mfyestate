...
<html>
  ...
  <!-- 3.添加class="layoutTab"  -->
   ...
   <table width="100%" border="0" cellpadding="0" cellspacing="1" bgcolor="#CCCCCC">
      <tr>
        <td height="30" align="left" bgcolor="#9EC4ED" ><STRONG><font size="3px">&nbsp;我备选的楼盘</font></STRONG> <a href="#" onclick="deleteAllproInfo();"> (点击清空)</a></td>
      </tr>
      <tr>
		              ...
		              <div class="left w548" style="border: 0px red solid;">
		                  <div class="right" style="border: 0px red solid; width: 228px;">
		                      ...
		                      <s:if test='#item.isAttention=="1"'>
		                          <img src="<%=basePath%>dynamic/img/white.gif" id=""/><img src="<%=basePath%>dynamic/img/white.gif" />
		                      </s:if>
		                      <s:else>    
	                              <img src="<%=basePath%>dynamic/img/white.gif" /><a href="#" style="cursor:pointer" onclick="setState('<s:property value="id"/>')" title="加入关注" rel="nofollow"><img src="<%=basePath%>dynamic/img/attention.gif" id="pic_<s:property value="id"/>"/></a>
                              </s:else>
	                              <input type="button" value="对 比" onclick="openCompare('<s:property value="name"/>','<s:property value="id"/>','<%=basePath%><s:property value="compressUrl"/>',1)" /><input type="button" value="删 除" onclick="delbuilding('<s:property value="id"/>')"/>                                    
...
<s:if test="%{currentPage!=1}">
	<a href="<%=basePath%>proInfo/proInfo_showProInfo.shtml?operateType=4&page=1">首页</a>
</s:if>
<s:else>
	<font disabled="true">首页</font>
</s:else>
<s:if test="%{currentPage<rowCount}">
	<a href="<%=basePath%>proInfo/proInfo_showProInfo.shtml?operateType=4&page=${currentPage+1}">下一页</a>
</s:if>
<s:else>
	<font disabled="true">下一页</font>
</s:else>
<s:if test="%{currentPage!=1}">
	<a href="<%=basePath%>proInfo/proInfo_showProInfo.shtml?operateType=4&page=${currentPage-1}">上一页</a>
</s:if>
<s:else>
	<font disabled="true">上一页</font>
</s:else>
<s:if test="%{currentPage<rowCount}">
<a href="<%=basePath%>proInfo/proInfo_showProInfo.shtml?operateType=4&page=${rowCount}">尾页</a>
</s:if>
<s:else>
  ...
  <script type="text/javascript">
   var basePath = '<%=basePath%>';
  	  function gotoPage(){
	  	...
	  	window.location.href="<%=basePath%>proInfo/proInfo_showProInfo.shtml?operateType=4&page="+$("#gotoPage").val();
	  }
   ...
   function setState(id){
		...
		$.get("<%=basePath%>proInfo/proInfo_createproInfo.shtml?operateType=4&proType=6&operateToType=2&proId="+id+"&time="+(new Date().getTime().toString(36)),function(obj){
		});
   }
   function delbuilding(id){
   	...
   	// alert(id)
   		$("#building_"+id).remove();
   		$.get("<%=basePath%>proInfo/proInfo_deleteproInfo.shtml?operateType=4&proType=6&proId="+id+"&time="+(new Date().getTime().toString(36)),function(obj){
		});
   }
   function deleteAllproInfo(){
   	$.messager.confirm('清空记录', '确定要删除所有备选的楼盘吗?', function(btn) {
		if (btn) {
	   		window.location.href="<%=basePath%>proInfo/proInfo_deleteAllproInfo.shtml?operateType=4&proType=6";
  		}
  	});
   }
  </script>
