...
function pagination(config){
	if(config){
		...
		var id = config.id;
		var width = 55;
		var nationStr = '<div id="pagenation_bar" class="pagenation"><span class="total_records">'+totalRecords+'</span>';
		if(totalPages <= showPages){
			...
			nationStr += '<span class="goto"><input onblur="dealUri(this.value,'+pageSize+','+((typeof uri == 'string')?'\''+uri+'\'':uri)+')" onkeydown="if(event.keyCode == 13)dealUri(this.value,'+pageSize+','+((typeof uri == 'string')?'\''+uri+'\'':uri)+')"/></span>';
			width += 100;
		}
