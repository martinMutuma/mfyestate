...
<html>
	<head>
		...
		<link rel="stylesheet" href="<%=basePath%>resource/css/global.css"
			type="text/css"></link>
		<link rel="stylesheet"
			href="<%=basePath%>resource/css/pagination2.css" type="text/css"></link>
		<link rel="stylesheet" href="<%=basePath%>resource/css/search.css"
			type="text/css"></link>
		<script type="text/javascript"
			src="<%=basePath%>resource/js/jquery-1.4.2.js" charset="utf-8"></script>
		<script type="text/javascript" src="<%=basePath%>resource/js/whole.js"></script>
		<script type="text/javascript"
			src="<%=basePath%>resource/js/search.js"></script>
		<script type="text/javascript"
			src="<%=basePath%>resource/js/pagination2.js"></script>
							...
							<ul style="width: 100%; position: absolute;">
								<s:iterator value="#proList" status="status" var="proTemp">
									<li style='clear: none; position: absolute; top: <s:property value = "#status.index*40"/>px;'>
										<div class="search_pro_bg"
											id="pro_<s:property value="#status.index"/>" style="">
											<s:property value="name" />
										</div>
										<div id="searchDiv_<s:property value="#status.index"/>"
											style='position: absolute; top: -<s:property value="#status.index*40"/>px; width: 805px; left: 150px; height: 200px; <s:if test = "#status.index != 0" > 
    									 display : none </s:if>'>
											<table border="0" cellspacing="0" style="margin-top: 5px;">
												<tr>
													...
													<td>
														<div class="keyWord_div" style="float: left;">
															<input id="keyword_<s:property value="#status.index"/>"
																style="width: 300px; height: 25px;" />
															&nbsp;&nbsp;
															<input type="button"
																onclick="searchKeyWord('<s:property value="#status.index"/>')"
																value="搜索" />
															<label>
																<input id="zj_<s:property value="#status.index"/>"
																	type="checkbox" value="1" checked="checked" />
																中介
															</label>
															<label>
																<input id="fzj_<s:property value="#status.index"/>"
												...
												<s:iterator value="searchList" status="seStatus"
													var="define">
													<tr>
														<td class="search_column_span">
															<s:property value="name" />
															：
														</td>
														<td>
															<span style="cursor: pointer"
																id="rc_<s:property value="#status.index"/>_<s:property value="#seStatus.index"/>_0"
																class="search_column search_column_selected"
																onclick="search('0','','','',this,'<s:property value="#seStatus.index"/>_0','<s:property value="#status.index"/>')">不限</span>
															<s:if test='%{searchType == "7"}'>
																<s:iterator value="dataList" status="ssStatus">
																	<s:if test='maxvalue==null||maxvalue==""'>
																		<span
																			id="rc_<s:property value="#status.index"/>_<s:property value="#seStatus.index"/>_<s:property value="#ssStatus.index+1"/>"
																			class="search_column"
																			onclick="search('<s:property value="searchType"/>','<s:property value="minvalue"/>','<s:property value="maxvalue"/>','<s:property value="#define.code"/>',this,'<s:property value="#seStatus.index"/>_<s:property value="#ssStatus.index+1"/>','<s:property value="#status.index"/>')"
																			style="cursor: pointer"><s:property
																				value="name" />
																		</span>
																	</s:if>
															...
															<s:else>
																<s:iterator value="dataList" status="ssStatus">
																	<span
																		id="rc_<s:property value="#status.index"/>_<s:property value="#seStatus.index"/>_<s:property value="#ssStatus.index+1"/>"
																		class="search_column"
																		onclick="search('<s:property value="searchType"/>','<s:property value="minvalue"/>','','<s:property value="#define.code"/>',this,'<s:property value="#seStatus.index"/>_<s:property value="#ssStatus.index+1"/>','<s:property value="#status.index"/>')"
																		style="cursor: pointer"><s:property
																			value="name" />
																	</span>
																</s:iterator>
