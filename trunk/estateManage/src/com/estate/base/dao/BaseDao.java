...
public class BaseDao<ENTITY, ID extends Serializable> implements
	...
	public List<ENTITY> findList(String Sql, Object[] objArray) {
		...
		if (sqlTmp.trim().equals("*")||sqlTmp.contains(".*")==true) {
			int length = fieldMap.size();
			String[] conditionArray = new String[length];
			int num = 0;
			for (String key : fieldMap.keySet()) {
				conditionArray[num] = key;
				num++;
			}
			...
			List<Map<String, Object>> list = null;
			if (null == objArray)
				list = (List<Map<String, Object>>) support.find(Sql, sqlArray);
			else
				list = (List<Map<String, Object>>) support.find(Sql, objArray,
						sqlArray);
			if (null != list && list.size() > 0) {
	...
	public List<Object[]> findObjectArryList(String sql, Object[] objArray) {
		...
		if (sqlTmp.trim().equals("*")) {
			int length = fieldMap.size();
			if (null == objArray)
				list = support.findObjList(sql, length);
			else
				list = support.findObjList(sql, objArray, length);
		} else {
			String[] sqlArray = sqlTmp.split(",");
			sqlArray = bulidMap(sqlArray);
	...
	private String[] bulidMapSimple(String[] conditionArray) {
		int length = conditionArray.length;
		String[] buf = new String[length];
		for (int j = 0; j < conditionArray.length; j++) {
			String coditionName = conditionArray[j].toLowerCase();
	...
	public List<Map<String, Object>> findAll(String Sql, Object[] objAry) {
		...
		String sqlTmp = Sql.substring(6, Sql.indexOf("from"));
		String[] sqlArray = sqlTmp.split(",");
		sqlArray = bulidMapSimple(sqlArray);
		if(null!=objAry){
