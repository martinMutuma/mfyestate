...
public class OfficeNeedServiceImpl implements OfficeNeedService {
       ...
       public   List<OfficeNeed>  getAllList(Map<String, String> map,String start,String limt){
    	   ...
    	   String  sql="select *  from  t_p_officeneed where  1=1  and needType="+map.get("type");
  	     if(null!=flag && (!"".equals(flag)) && !("0").equals(flag)) sql+=" and  flag= "+flag;
  	     if(null!=auting &&  (!"-1".equals(auting ))  )sql+=" and  auditing="+auting;
  	      ...
  	      if(list!=null &&  list.size()>0){
  	    	  ...
  	    	  for(OfficeNeed need:list){
  	    		  if(need.getMaxPrice()!=0.0){
  	    		    need.setPriceString(need.getMinPrice()+"-"+need.getMaxPrice());
  	    		  }else    need.setPriceString(need.getMinPrice()+"");
  	    		  need.setCreateTimeString( DateTimeUtil.switchDateToString(need.getCreateTime(),"yyyy-MM-dd hh:mm:ss"));
  	    		  if("1".equals(need.getFlag())) need.setFlagString("求租");
  	    		  if("2".equals(need.getFlag())) need.setFlagString("求购");
  	    		  if("0".equals(need.getAuditing()))need.setAudtingString("未审核");
  	    		  if("1".equals(need.getAuditing()))need.setAudtingString("已审核");
  	    		  if("5".equals(need.getAuditing()))need.setAudtingString("审核驳回");
  	    		  newList.add(need);
  	    	  }
  	      }
  	      return newList;
       ...
       public  int getcount(Map<String,String> map){
    	   ...
    	   String  flag=map.get("flag");
    	   String  auting=map.get("auting");
    	   String  sql="select * from  t_p_officeneed where  1=1 and  needType="+type;
    	   if(null!=flag && (!"".equals(flag)) && !("0").equals(flag)) sql+=" and  flag= "+flag;
  	      if(null!=auting &&  (!"-1".equals(auting ))  )sql+=" and  auditing="+auting;
  	      try {
  	    	  List<OfficeNeed> list=dao.findList(sql);
       ...
       public boolean deleteByIdString(String idString ,String type) {
   		...
   		try {
   			for (int i = 0; i < delteid.length; i++) {
   				sql = "delete   from  t_p_officeneed where  id=" + delteid[i]+ "   and  needType= "+type;
   				dao.delete(sql);
   			}
   		} catch (Exception e) {
   			e.printStackTrace();
       ...
       public   OfficeNeed  getOfficeNeedById(String  id,String  type){
    	   ...
    	   try {
    		   List<OfficeNeed>   list=  dao.findList(sql, new Object[]{id,type});   
