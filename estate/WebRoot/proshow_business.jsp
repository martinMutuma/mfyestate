...
<script type="text/javascript" src="<%=basePath %>proShow.js"></script>
  ...
  <body  onload="load()" onunload="GUnload()">
  	...
  	<div style="width: 100%;" align="center">
  	    <div class="l_loca">
    	<span class="left lo_le">您当前的位置：<a href="home.shtml">梦房园</a>&nbsp;&gt;&gt;&nbsp;<a href="search.shtml?searchPro=2"><estateTag:convertCode codeType="导航" code="${menuIndex}"></estateTag:convertCode></a>&nbsp;&gt;&gt;&nbsp;<s:property value="dataMap.title"/></span>
       </div>
	   				...
	   				<div id="baseinfo">
	   					...
	   					<div id="biRight">
	   						<table width="100%" cellspacing="0" border="0" cellpadding="0">
	   							<tr>
	   								<td valign="bottom" colspan="2">行业类别：<s:if test='%{dataMap.category=="1"}'>餐饮美食</s:if><s:elseif test='%{dataMap.category=="2"}'>娱乐休闲</s:elseif><s:elseif test='%{dataMap.category=="3"}'>百货零售</s:elseif><s:elseif test='%{dataMap.category=="4"}'>公司工厂</s:elseif><s:else>其他服务业</s:else>
	   								</td>
	   							</tr>
	   							<tr>
	   							...
	   							<tr>
	   								<td valign="bottom" colspan="2">转让价格：<estateTag:format value="${dataMap.price}"></estateTag:format> 万元</td>
	   							</tr>
	   							<tr>
	   								<td valign="bottom" width="50%">物业面积：<estateTag:format value="${dataMap.area}"/>㎡</td>
	   								<td valign="bottom">房源持有状态状况：
	   									<estateTag:convertCode codeType="物业状况" code="${dataMap.state}"></estateTag:convertCode>
	   								</td>
	   							</tr>
	   							<s:if test='%{dataMap.state == "1"}'>
	   							<tr>
	   								<td valign="bottom">租&emsp;&emsp;金：<estateTag:format value="${dataMap.propertyRent}"></estateTag:format>元/月 </td>
	   								<td valign="bottom">租赁到期日：<s:date name="dataMap.rentDate" format="yyyy-MM-dd"/></td>
	   							</tr>
	   							<tr>
	   								<td valign="bottom">租金付款方式：<estateTag:convertCode codeType="租赁支付方式" code="${dataMap.payWay}"></estateTag:convertCode>元/月 </td>
	   								<td valign="bottom">租金递增状况：<s:property value="dataMap.rentState"/></td>
	   							</tr>
	   							</s:if>
	   						</table>
	   					</div>
	   				</div>
	   				<div align="right" id="company">
	   					<div class="instroduce" align="left">经营状况</div>
	   					<div class="border_full instroduceContent" align="center">
	   					<table width="95%" border="0">
	   						<tr>
	   							<td width="25%" align="right">公司类型：</td><td width="25%" align="left"><estateTag:convertCode codeType="公司类型" code="${dataMap.companyType}"></estateTag:convertCode></td>
	   							<td width="25%" align="right">注册资金：</td><td align="left"><estateTag:format value="${dataMap.capital}"></estateTag:format>万元</td>
	   						</tr>
	   						<tr>	
	   							<td align="right">转让股份：</td><td align="left"><s:property value="dataMap.shares"/>%</td>
	   							<td align="right">经营范围：</td><td align="left"><s:property value="dataMap.scope"/></td>
	   						</tr>
	   						<tr>	
	   							<td align="right">已经营年限：</td><td align="left"><s:property value="dataMap.businessYear"/></td>
	   							<td align="right">过去一年营业额：</td><td align="left"><estateTag:format value="${dataMap.businessturnover}"></estateTag:format>万元</td>
	   						</tr>
	   						<tr>	
	   							<td align="right">过去一年利润：</td><td align="left"><estateTag:format value="${dataMap.businessprofits}"></estateTag:format>万元</td>
	   							<td align="right">过去一年营业税：</td><td align="left"><estateTag:format value="${dataMap.businessTax}"></estateTag:format>万元</td>
	   						</tr>
	   						<tr>	
	   							<td align="right">过去一年所得税：</td><td align="left"><estateTag:format value="${dataMap.incomeTax}"></estateTag:format>万元</td>
	   							<td align="right">是否会员制：</td><td align="left"><s:if test='%{dataMap.isMember == "1"}'>是</s:if><s:else>否</s:else></td>
	   						</tr>
	   					</table>
	   					</div>
	   				</div>
