$(function(){
		searchObj = $("#pro_0");
  		...
  		if(proId!=""){
			...
			searchDiv = queryObj;
	  		...
	  		for(var i = 0; i < stausAry.length; i++){
	  			...
	  			if(columnIndex!="0")
	  				$("#rc_"+proId+"_"+rowIndex+"_0").removeClass("search_column_selected");
	  			$("#rc_"+proId+"_"+rowIndex+"_"+columnIndex).addClass("search_column_selected");
		  		//初始化选择项
	  		}
	  	...
	  	if(document.getElementById("bulidListDiv")!=null){
	  		$("#bulidListDiv table").not( $("#bulidListDiv table table")).hover(function(){
	  	...
	  	if(document.getElementById("lptsValue")!=null){
			...
			if(lpts==""){
				$("#lpts_0").css({color:'#FFA028'});
			}else{
				lpts = parseInt(lpts);
				$("#lpts_"+(lpts+1)).css({color:'#FFA028'});
			}
	  	}
  	})
  	...
  	*/
  	function search(type,minValue,maxValue,code,obj,position,pro){
  		...
  		$("#searchPro").val(pro);
  		if($("#searchStatus").val()!=""){
  			filteQuery(position,(code+'~'+type+'~'+minValue+'~'+maxValue));
  		}else{
  			$("#searchStatus").val(position);
  			$("#searchQuery").val((code+'~'+type+'~'+minValue+'~'+maxValue));
  		}
 ...
 /*	
  	function proSearch(pro){
  		...
  		$("#searchPro").val(pro);
  		$("#keywordValue").val($("#keyword_"+pro).val());
  	...
  	*/
  	...
  	function keyWord(keyword,item,pageno){
		go(project_root_path+'keyWordSearch.shtml?keyword='+keyword+'&proIndex='+item+'&pageNo='+pageno);
  	}
...
function searchByPrice(minValue,maxValue,type,proType,pageno){
	window.location.href='searchByPrice.shtml?minValue='+minValue+'&maxValue='+maxValue+'&type='+type+'&proType='+proType+'&pageNo='+pageno;
}
