...
<html>
  ...
  <!-- 3.添加class="layoutTab"  -->
   ...
   <table width="100%" border="0" cellpadding="0" cellspacing="1" bgcolor="#CCCCCC">
      <tr>
        <td height="30" align="left" bgcolor="#9EC4ED" ><STRONG><font size="3px">&nbsp;我关注的楼盘</font></STRONG> <a href="#" onclick="deleteAllproInfo();"> (点击清空)</a></td>
      </tr>
      <tr>
                  ...
                  <s:iterator value="tbuildingList" status="st" id="item">
		              ...
		              <div class="left w548" style="border: 0px red solid;">
		                  <div class="right" style="border: 0px red solid; width: 228px;">
		                      ...
		                      <s:else>    
	                              <img src="<%=basePath%>dynamic/img/white.gif" /><a href="#" style="cursor:pointer" onclick="setState('<s:property value="id"/>','4')" title="加入备选" rel="nofollow"><img src="<%=basePath%>dynamic/img/choose.gif" id="pic_<s:property value="id"/>"/></a>
                              </s:else>
...
<s:if test="%{currentPage!=1}">
	<a href="<%=basePath%>proInfo/proInfo_showProInfo.shtml?operateType=2&page=1">首页</a>
</s:if>
<s:else>
	<font disabled="true">首页</font>
</s:else>
<s:if test="%{currentPage<rowCount}">
	<a href="<%=basePath%>proInfo/proInfo_showProInfo.shtml?operateType=2&page=${currentPage+1}">下一页</a>
</s:if>
<s:else>
	<font disabled="true">下一页</font>
</s:else>
<s:if test="%{currentPage!=1}">
	<a href="<%=basePath%>proInfo/proInfo_showProInfo.shtml?operateType=2&page=${currentPage-1}">上一页</a>
</s:if>
<s:else>
	<font disabled="true">上一页</font>
</s:else>
<s:if test="%{currentPage<rowCount}">
<a href="<%=basePath%>proInfo/proInfo_showProInfo.shtml?operateType=2&page=${rowCount}">尾页</a>
</s:if>
<s:else>
...
转到第:<input type="text" id="gotoPage" name="page" size="8"/> 页 <input type="button" onclick="gotoPage();" value="GO" name="cndok" <s:if test="%{rowCount==1}"> disabled="true" </s:if>/>
  ...
  var basePath = '<%=basePath%>';
  	  function gotoPage(){
	  	...
	  	window.location.href="<%=basePath%>proInfo/proInfo_showProInfo.shtml?operateType=2&page="+$("#gotoPage").val();
	  }
   ...
   function setState(id,optype){
		...
		$.get("<%=basePath%>proInfo/proInfo_createproInfo.shtml?operateType=2&proType=6&operateToType="+optype+"&proId="+id+"&time="+(new Date().getTime().toString(36)),function(obj){
		});
   }
   function delbuilding(id){
   	...
   	// alert(id)
   		$("#building_"+id).remove();
   		$.get("<%=basePath%>proInfo/proInfo_deleteproInfo.shtml?operateType=2&proType=6&proId="+id+"&time="+(new Date().getTime().toString(36)),function(obj){
		});
   }
   function deleteAllproInfo(){
   	$.messager.confirm('清空记录', '确定要删除所有关注的楼盘吗?', function(btn) {
		if (btn) {
	   		window.location.href="<%=basePath%>proInfo/proInfo_deleteAllproInfo.shtml?operateType=2&proType=6";
  		}
  	});
   }
  </script>
